<template>
  ...
  <div class="grid grid-flow-row grid-cols-2 2xl:grid-cols-4 gap-5">
    <div v-for="item in data.slice(0,4)" :key="item.id"
        ...
        <div class="text-xs text-gray-600 mt-2">
          <div v-for="(track,index) in item.tracks" class="mt-2">
            <div class="flex">
              ...
              <n-ellipsis class="flex-auto w-20">
  ...
  <div class="grid grid-flow-row grid-cols-6 2xl:grid-cols-12 gap-5">
    <div v-for="item in data.slice(4)" :key="item.id" @click="toPlaylist(item)">
...
onMounted(async () => {
  data.value = await useTopListDetail()
})
